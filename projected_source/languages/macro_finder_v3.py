...
class MacroFinder:
    ...
    def find_by_name(self, source: bytes, name: str) -> List[MacroResult]:
        ...
        return self._execute_query(
            source,
            self._build_query(PredicateType.EQUALS, name)
        )
    ...
    def find_by_pattern(self, source: bytes, pattern: str) -> List[MacroResult]:
        ...
        return self._execute_query(
            source,
            self._build_query(PredicateType.MATCH, pattern)
        )
    
    def find_by_argument(self, source: bytes, name: str, 
                        arg_pos: int, arg_val: str) -> List[MacroResult]:
        """Find macros where specific argument has specific value."""
        def arg_filter(result: MacroResult) -> bool:
            args = result['arguments']
            return (arg_pos < len(args) and 
                   args[arg_pos].strip() == arg_val)
        
        return self._execute_query(
            source,
            self._build_query(PredicateType.EQUALS, name),
            filter_fn=arg_filter
        )
    ...
    def _execute_query(self, source: bytes, query_text: str,
                      filter_fn: Optional[Callable[[MacroResult], bool]] = None) -> List[MacroResult]:
    ...
    def _process_matches(self, matches, 
        ...
        for pattern_index, captures in matches:
            ...
            result = self._build_result(
                macro_nodes[0],
                args_nodes[0],
                macro_name_nodes[0] if macro_name_nodes else None
            )
    ...
    def _build_result(self, macro_node: Node, args_node: Node,
                     name_node: Optional[Node] = None) -> MacroResult:
